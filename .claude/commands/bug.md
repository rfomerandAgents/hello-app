...
- `cd app/ && bun tsc --noEmit` - Run TypeScript type check to validate the bug is fixed with zero regressions
- `cd app/ && bun run lint` - Run Next.js linting to validate code quality
- `cd app/ && bun run build` - Run production build to validate the bug is fixed with zero regressions
...
- Example: `specs/issue-123-adw-abc12345-sdlc_planner-my-bug.md`
