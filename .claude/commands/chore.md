...
- CRITICAL: Create the plan file using ONLY the relative path: `specs/issue-{issue_number}-adw-{adw_id}-sdlc_planner-{descriptive-name}.md`
...
- `asw/app/*.py` contain astral uv single file python scripts. So if you want to run them use `uv run <script_name>`.
...
- `cd app/ && bun tsc --noEmit` - Run TypeScript type check to validate the chore is complete with zero regressions
- `cd app/ && bun run lint` - Run Next.js linting to validate code quality
- `cd app/ && bun run build` - Run production build to validate the chore is complete with zero regressions
...
- Example: `specs/issue-123-adw-abc12345-sdlc_planner-my-chore.md`
