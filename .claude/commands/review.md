...
- IMPORTANT: If the work can be validated by UI validation then (if not skip the section):
  ...
  - IMPORTANT: Take screen shots along the way to showcase the new functionality and any issues you find
    ...
    - IMPORTANT: When using Playwright MCP to take screenshots, you will receive base64-encoded image data
      - CRITICAL: You MUST save these base64 screenshots to actual files on disk before including in the report:
        1. Create the review_image_dir if it doesn't exist: `mkdir -p <review_image_dir>`
        2. For each screenshot from Playwright MCP:
           - Extract the base64 data from the response (after the `data:image/png;base64,` prefix)
           - Save it to a file using: `echo "<base64_data>" | base64 -d > <review_image_dir>/<filename>.png`
           - Or use Python: `python3 -c "import base64; open('<path>', 'wb').write(base64.b64decode('<data>'))"`
        3. Use the ABSOLUTE FILE PATH in the screenshots array, NOT the base64 data URI
      - The screenshot paths should look like: `/absolute/path/to/agents/<adw_id>/<agent_name>/review_img/01_feature.png`
      - Do NOT embed base64 data URIs in the JSON output - GitHub cannot render them
...
- The {{PROJECT_NAME}} website will be accessible at http://localhost:3000 (Next.js default port)
...
```json
{
    ...
    review_summary: "string - 2-4 sentences describing what was built and whether it matches the spec. Written as if reporting during a standup meeting. Example: 'The new contact form has been implemented with email validation and spam protection. The implementation matches the spec requirements for accessibility and responsive design. Minor styling improvements could be made but all core functionality is working as specified.'",
    review_issues: [
        {
