...
Provides utilities for creating and managing git worktrees under trees/<ipe_id>/
...
def create_worktree(ipe_id: str, branch_name: str, logger: logging.Logger) -> Tuple[str, Optional[str]]:
    """Create a git worktree for isolated ADW execution.
    
    Args:
        ipe_id: The ADW ID for this worktree
    ...
    worktree_path = os.path.join(trees_dir, ipe_id)
...
def validate_worktree(ipe_id: str, state: IPEState) -> Tuple[bool, Optional[str]]:
...
def get_worktree_path(ipe_id: str) -> str:
    ...
    return os.path.join(project_root, "trees", ipe_id)


def remove_worktree(ipe_id: str, logger: logging.Logger) -> Tuple[bool, Optional[str]]:
    """Remove a worktree and clean up.
    
    Args:
        ipe_id: The ADW ID for the worktree to remove
        logger: Logger instance
        
    Returns:
        Tuple of (success, error_message)
    """
    worktree_path = get_worktree_path(ipe_id)
...
def get_ports_for_adw(ipe_id: str) -> Tuple[int, int]:
...
def find_next_available_ports(ipe_id: str, max_attempts: int = 15) -> Tuple[int, int]:
    ...
    """
    base_backend, base_frontend = get_ports_for_adw(ipe_id)
