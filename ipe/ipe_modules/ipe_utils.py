"""Utility functions for IPE (Infrastructure Platform Engineer) system."""
...
def make_ipe_id() -> str:
    """Generate a short 8-character UUID for IPE tracking."""
    return str(uuid.uuid4())[:8]


def setup_logger(ipe_id: str, trigger_type: str = "ipe_plan_iso") -> logging.Logger:
    """Set up logger that writes to both console and file using ipe_id.

    Args:
        ipe_id: The IPE workflow ID
        trigger_type: Type of trigger (ipe_plan_iso, trigger_webhook, etc.)

    Returns:
        Configured logger instance
    """
    # Create log directory: agents/{ipe_id}/ipe_plan_iso/
    # __file__ is in ipe/ipe_modules/, so we need to go up 3 levels to get to project root
    project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    log_dir = os.path.join(project_root, "agents", ipe_id, trigger_type)
    os.makedirs(log_dir, exist_ok=True)

    # Log file path: agents/{ipe_id}/ipe_plan_iso/execution.log
    log_file = os.path.join(log_dir, "execution.log")

    # Create logger with unique name using ipe_id
...
def get_logger(ipe_id: str) -> logging.Logger:
    """Get existing logger by IPE ID.
    ...
    Returns:
        Logger instance
    """
    return logging.getLogger(f"ipe_{ipe_id}")
...
def get_safe_subprocess_env() -> Dict[str, str]:
    ...
    Returns only the environment variables needed for IPE workflows based on
