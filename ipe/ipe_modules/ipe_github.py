...
"""
GitHub Operations Module - AI Developer Workflow (IPE)
...
from .ipe_data_types import GitHubIssue, GitHubIssueListItem, GitHubComment

# Bot identifier to prevent webhook loops and filter bot comments
IPE_BOT_IDENTIFIER = "[IPE-AGENTS]"
...
def make_issue_comment(issue_id: str, comment: str) -> None:
    ...
    # Ensure comment has IPE_BOT_IDENTIFIER to prevent webhook loops
    if not comment.startswith(IPE_BOT_IDENTIFIER):
        comment = f"{IPE_BOT_IDENTIFIER} {comment}"
...
def mark_issue_in_progress(issue_id: str) -> None:
    ...
    # make_issue_comment(issue_id, "🚧 IPE is working on this issue...")
...
def find_keyword_from_comment(keyword: str, issue: GitHubIssue) -> Optional[GitHubComment]:
    ...
    for comment in sorted_comments:
        # Skip IPE bot comments to prevent loops
        if IPE_BOT_IDENTIFIER in comment.body:
            continue
