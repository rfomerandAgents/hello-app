...
SLASH_COMMAND_MODEL_MAP: Final[Dict[SlashCommand, Dict[ModelSet, str]]] = {
    "/classify_ipe": {"base": "sonnet", "heavy": "opus"},
    "/generate_branch_name": {"base": "sonnet", "heavy": "opus"},
    "/ipe_build": {"base": "sonnet", "heavy": "opus"},
    "/ipe_test": {"base": "sonnet", "heavy": "opus"},
    "/resolve_failed_test": {"base": "sonnet", "heavy": "opus"},
    "/ipe_review": {"base": "sonnet", "heavy": "opus"},
    "/ipe_document": {"base": "sonnet", "heavy": "opus"},
    "/ipe_commit": {"base": "sonnet", "heavy": "opus"},
    "/ipe_pull_request": {"base": "sonnet", "heavy": "opus"},
    "/ipe_chore": {"base": "sonnet", "heavy": "opus"},
    "/ipe_bug": {"base": "sonnet", "heavy": "opus"},
    "/ipe_feature": {"base": "sonnet", "heavy": "opus"},
    "/ipe_patch": {"base": "sonnet", "heavy": "opus"},
    "/install_worktree": {"base": "sonnet", "heavy": "opus"},
    "/ipe_track_agentic_kpis": {"base": "sonnet", "heavy": "opus"},
}
...
def get_model_for_slash_command(
    ...
    Args:
        request: The template request containing the slash command and ipe_id
    ...
    state = IPEState.load(request.ipe_id)
    if state:
...
def get_claude_env() -> Dict[str, str]:
    ...
    """
    # Import here to avoid circular imports
    from .ipe_utils import get_safe_subprocess_env

    # Use the shared function
    return get_safe_subprocess_env()


def save_prompt(prompt: str, ipe_id: str, agent_name: str = "ops") -> None:
    ...
    prompt_dir = os.path.join(project_root, "agents", ipe_id, agent_name, "prompts")
...
def prompt_claude_code(request: AgentPromptRequest) -> AgentPromptResponse:
    ...
    save_prompt(request.prompt, request.ipe_id, request.agent_name)
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if result_message:
                # Extract session_id and duration_ms from result message
...
def execute_template(request: AgentTemplateRequest) -> AgentPromptResponse:
    ...
    # Create output directory with ipe_id at project root
    # __file__ is in adws/adw_modules/, so we need to go up 3 levels to get to project root
    project_root = os.path.dirname(
        os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    )
    output_dir = os.path.join(
        project_root, "agents", request.ipe_id, request.agent_name
    )
    os.makedirs(output_dir, exist_ok=True)
