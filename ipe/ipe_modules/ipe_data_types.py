"""Data types for Infrastructure Platform Engineer (IPE) workflows."""
...
IssueClassSlashCommand = Literal["/ipe_chore", "/ipe_bug", "/ipe_feature"]

# Model set types for IPE workflows
ModelSet = Literal["base", "heavy"]

# IPE workflow types (all isolated)
IPEWorkflow = Literal[
    "ipe_plan_iso",  # Planning only
    "ipe_build_iso",  # Building infrastructure
    "ipe_test_iso",  # Testing infrastructure
    "ipe_review_iso",  # Review infrastructure changes
    "ipe_document_iso",  # Documentation
    "ipe_ship_iso",  # Ship/deployment workflow
]

# All slash commands used in the IPE system
# Includes issue classification commands and IPE-specific commands
...
class AgentPromptRequest(BaseModel):
    """Claude Code agent prompt configuration for IPE."""

    prompt: str
    ipe_id: str  # Changed from adw_id
    agent_name: str = "ops"
...
class AgentTemplateRequest(BaseModel):
    """Claude Code agent template execution request for IPE."""
...
class IPEStateData(BaseModel):
    """Minimal persistent state for IPE workflow.

    Stored in agents/{ipe_id}/ipe_state.json
    ...
    spec_file: Optional[str] = None  # Changed from plan_file
    ...
    all_ipes: List[str] = Field(default_factory=list)  # Changed from all_adws
    # REMOVED: backend_port, frontend_port (application-specific)


class ReviewIssue(BaseModel):
    """Individual review issue found during infrastructure verification."""

    review_issue_number: int
    screenshot_path: str  # Local file path to screenshot (e.g., "agents/IPE-123/reviewer/review_img/error.png")
    screenshot_url: Optional[str] = (
        None  # Public URL after upload (e.g., "https://domain.com/ipe/IPE-123/review/error.png")
    )
...
class ReviewResult(BaseModel):
    """Result from reviewing infrastructure changes against specification."""

    success: bool
    review_summary: (
        str  # 2-4 sentences describing what infrastructure was built and whether it matches the spec
    )
    review_issues: List[ReviewIssue] = []
    screenshots: List[str] = (
        []
    )  # Local file paths (e.g., ["agents/IPE-123/reviewer/review_img/ui.png"])
...
class IPEExtractionResult(BaseModel):
    """Result from extracting IPE information from text."""

    workflow_command: Optional[str] = None  # e.g., "ipe_plan_iso" (without slash)
    ipe_id: Optional[str] = None  # 8-character IPE ID
