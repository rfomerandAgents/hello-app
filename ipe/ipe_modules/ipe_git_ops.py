"""Git operations for IPE composable architecture.
...
from .ipe_github import get_repo_url, extract_repo_path, make_issue_comment
...
def approve_pr(pr_number: str, logger: logging.Logger) -> Tuple[bool, Optional[str]]:
    ...
    result = subprocess.run(
        [
            ...
            "--approve",
            "--body",
            "IPE Ship workflow approved this PR after validating all state fields.",
        ],
        capture_output=True,
        text=True,
    )
...
def merge_pr(
    ...
    # Add auto-merge body
    merge_cmd.extend(
        ["--body", "Merged by IPE Ship workflow after successful validation."]
    )
...
def finalize_git_operations(
    state: "IPEState", logger: logging.Logger, cwd: Optional[str] = None
) -> None:
    ...
    if pr_url:
        ...
        if issue_number and ipe_id:
            make_issue_comment(issue_number, f"{ipe_id}_ops: ✅ Pull request: {pr_url}")
    else:
        ...
        if issue_number:
            try:
                repo_url = get_repo_url()
                repo_path = extract_repo_path(repo_url)
                from .ipe_github import fetch_issue

                issue = fetch_issue(issue_number, repo_path)

                from .ipe_workflow_ops import create_pull_request
        ...
        if pr_url:
            ...
            if issue_number and ipe_id:
                make_issue_comment(
                    issue_number, f"{ipe_id}_ops: ✅ Pull request created: {pr_url}"
                )
        else:
