"""Git operations for workflow composable architecture.
...
# Import GitHub functions from shared module
from shared.github import get_repo_url, extract_repo_path, make_issue_comment
...
def approve_pr(pr_number: str, logger: logging.Logger) -> Tuple[bool, Optional[str]]:
    ...
    result = subprocess.run(
        [
            ...
            "--approve",
            "--body",
            "Workflow approved this PR after validating all state fields.",
        ],
        capture_output=True,
        text=True,
    )
...
def merge_pr(
    ...
    # Add auto-merge body
    merge_cmd.extend(
        ["--body", "Merged by workflow after successful validation."]
    )
    ...
    if pr_url:
        ...
        if issue_number:
            try:
                repo_url = get_repo_url()
                repo_path = extract_repo_path(repo_url)
                from shared.github import fetch_issue

                issue = fetch_issue(issue_number, repo_path)

                # Import workflow_ops from unified ASW or legacy modules
                try:
                    from asw.modules.workflow_ops import create_pull_request
                except ImportError:
                    try:
                        from adw_modules.workflow_ops import create_pull_request
                    except ImportError:
                        from ipe.ipe_modules.ipe_workflow_ops import create_pull_request
