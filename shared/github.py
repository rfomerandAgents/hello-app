...
"""
GitHub Operations Module - Workflow System
...
# Import data types from unified ASW modules
try:
    from asw.modules import GitHubIssue, GitHubIssueListItem, GitHubComment
except ImportError:
    # Fallback for legacy context
...
WORKFLOW_BOT_IDENTIFIER = "[ASW-AGENTS]"  # Updated for unified ASW system
...
def make_issue_comment(issue_id: str, comment: str) -> None:
    ...
    # Ensure comment has WORKFLOW_BOT_IDENTIFIER to prevent webhook loops
    if not comment.startswith(WORKFLOW_BOT_IDENTIFIER):
        comment = f"{WORKFLOW_BOT_IDENTIFIER} {comment}"
...
def find_keyword_from_comment(keyword: str, issue: GitHubIssue) -> Optional[GitHubComment]:
    ...
    for comment in sorted_comments:
        # Skip workflow bot comments to prevent loops
        if WORKFLOW_BOT_IDENTIFIER in comment.body:
            continue
