"""State management for workflow composable architecture.
...
try:
    from asw.modules import ASWAppStateData as ADWStateData  # Unified ASW type
except ImportError:
    from adw_modules.data_types import ADWStateData  # Legacy fallback


class WorkflowState:
    """Container for workflow state with file persistence.

    This is the shared base class. ADW and IDW can extend it.
    """

    STATE_FILENAME = "workflow_state.json"

    def __init__(self, workflow_id: str):
        """Initialize WorkflowState with a required workflow ID.

        Args:
            workflow_id: The workflow ID for this state (required)
        """
        if not workflow_id:
            raise ValueError("workflow_id is required for WorkflowState")

        self.workflow_id = workflow_id
        # Start with minimal state
        self.data: Dict[str, Any] = {"adw_id": self.workflow_id}  # Keep adw_id for backward compat
        self.logger = logging.getLogger(__name__)

    def update(self, **kwargs):
        """Update state with new key-value pairs."""
        # Filter to only our core fields
        core_fields = {"adw_id", "issue_number", "branch_name", "plan_file", "issue_class", "worktree_path", "backend_port", "frontend_port", "model_set", "all_adws", "environment"}
    ...
    def append_adw_id(self, adw_id: str):
        """Append an ADW/IDW ID to the all_adws list if not already present."""
    ...
    def get_working_directory(self) -> str:
        """Get the working directory for this workflow instance.
        ...
        # Return main repo path (parent of shared directory)
    ...
    def get_state_path(self) -> str:
        ...
        workflow_id = self.data.get("adw_id", self.workflow_id)
        return os.path.join(project_root, "agents", workflow_id, self.STATE_FILENAME)

    def save(self, workflow_step: Optional[str] = None) -> None:
        """Save state to file in agents/{workflow_id}/workflow_state.json."""
    ...
    @classmethod
    def load(
        cls, workflow_id: str, logger: Optional[logging.Logger] = None
        ...
        state_paths = [
            os.path.join(project_root, "agents", workflow_id, "adw_state.json"),
            os.path.join(project_root, "agents", workflow_id, cls.STATE_FILENAME),
        ]
...
class ADWState(WorkflowState):
    """ADW-specific state management (alias for WorkflowState)."""
