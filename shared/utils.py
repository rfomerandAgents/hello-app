"""Utility functions for workflow systems."""
...
def setup_logger(workflow_id: str, trigger_type: str = "workflow") -> logging.Logger:
    """Set up logger that writes to both console and file using workflow_id.

    Args:
        workflow_id: The workflow ID
        trigger_type: Type of trigger (adw_plan_build, idw_test_iso, etc.)

    Returns:
        Configured logger instance
    """
    # Create log directory: agents/{workflow_id}/{trigger_type}/
    project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    log_dir = os.path.join(project_root, "agents", workflow_id, trigger_type)
    os.makedirs(log_dir, exist_ok=True)

    # Log file path: agents/{workflow_id}/{trigger_type}/execution.log
    log_file = os.path.join(log_dir, "execution.log")

    # Create logger with unique name using workflow_id
    ...
    logger.info(f"Workflow Logger initialized - ID: {workflow_id}")
    logger.debug(f"Log file: {log_file}")

    return logger


def get_logger(workflow_id: str) -> logging.Logger:
    ...
    Returns:
        Logger instance
    """
    return logging.getLogger(f"workflow_{workflow_id}")
...
def get_safe_subprocess_env() -> Dict[str, str]:
    ...
    Returns only the environment variables needed for workflows based on
