...
"""
ASW App Patch Isolated - AI Developer Workflow for single-issue patches with worktree isolation

Usage:
  uv run asw_app_patch_iso.py <issue-number> [asw-id]
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: uv run asw_app_patch_iso.py <issue-number> [asw-id]")
        sys.exit(1)
    ...
    state.append_asw_id("adw_patch_iso")

    # Set up logger with ADW ID
    logger = setup_logger(adw_id, "adw_patch_iso")
    logger.info(f"ASW App Patch Isolated starting - ID: {adw_id}, Issue: {issue_number}")
