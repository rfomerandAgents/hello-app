...
"""
ASW App Plan Build Review Iso - Compositional workflow for isolated planning, building, and reviewing

Usage: uv run asw_app_plan_build_review_iso.py <issue-number> [asw-id] [--skip-resolution]

This script runs:
1. asw_app_plan_iso.py - Planning phase (isolated)
2. asw_app_build_iso.py - Implementation phase (isolated)
3. asw_app_review_iso.py - Review phase (isolated)
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: uv run asw_app_plan_build_review_iso.py <issue-number> [asw-id] [--skip-resolution]")
