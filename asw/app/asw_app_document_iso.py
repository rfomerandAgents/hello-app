...
"""
ASW App Document Iso - AI Developer Workflow for documentation generation in isolated worktrees

Usage:
  uv run asw_app_document_iso.py <issue-number> <asw-id>
...
def main():
    ...
    if len(sys.argv) < 3:
        print("Usage: uv run asw_app_document_iso.py <issue-number> <asw-id>")
    ...
    if state:
        ...
        logger.error(
            "Run asw_app_plan_iso.py or asw_app_patch_iso.py first to create the worktree and state"
        )
    ...
    state.append_asw_id("adw_document_iso")

    # Set up logger with ADW ID from command line
    logger = setup_logger(adw_id, "adw_document_iso")
    logger.info(f"ASW App Document Iso starting - ID: {adw_id}, Issue: {issue_number}")
