...
"""
ASW App Build Iso - AI Developer Workflow for agentic building in isolated worktrees

Usage:
  uv run asw_app_build_iso.py <issue-number> <asw-id> [--cache|--no-cache]
...
from asw.modules.git_ops import commit_changes, finalize_git_operations, get_current_branch
from asw.modules.github import fetch_issue, make_issue_comment, get_repo_url, extract_repo_path
...
from asw.modules.data_types import GitHubIssue
...
def main():
    ...
    if len(sys.argv) < 3:
        print("Usage: uv run asw_app_build_iso.py <issue-number> <asw-id> [--cache|--no-cache]")
    ...
    if state:
        ...
        logger.error("Run asw_app_plan_iso.py first to create the worktree and state")
        print(f"\nError: No state found for ADW ID: {adw_id}")
        print("Run asw_app_plan_iso.py first to create the worktree and state")
        sys.exit(1)
    
    # Track that this ADW workflow has run
    state.append_asw_id("adw_build_iso")
    
    # Set up logger with ADW ID from command line
    logger = setup_logger(adw_id, "adw_build_iso")
    logger.info(f"ASW App Build Iso starting - ID: {adw_id}, Issue: {issue_number}")
    ...
    if not state.get("branch_name"):
        error_msg = "No branch name in state - run asw_app_plan_iso.py first"
    ...
    if not state.get("plan_file"):
        error_msg = "No plan file in state - run asw_app_plan_iso.py first"
        logger.error(error_msg)
        make_issue_comment(
            issue_number,
            format_issue_message(adw_id, "ops", f"❌ {error_msg}")
    ...
    implement_response = implement_plan(plan_file, adw_id, logger, working_dir=worktree_path, cache_enabled=cache_enabled)
    ...
    commit_msg, error = create_commit(AGENT_IMPLEMENTOR, issue, issue_command, adw_id, logger, worktree_path, cache_enabled)
