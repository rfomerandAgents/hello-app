...
"""
ASW App Ship Iso - AI Developer Workflow for shipping (merging) to main

Usage:
  uv run asw_app_ship_iso.py <issue-number> <asw-id> [--skip-ci-check] [--create-ami]
...
Requirements:
  ...
  4. Complete ADW state with all required fields populated
...
from asw.modules.data_types import ADWStateData
...
def track_kpis_on_main(
    issue_number: str,
    adw_id: str,
    state: ADWState,
    logger: logging.Logger
) -> None:
    """Track agentic KPIs on main branch after successful merge.

    This runs on the main repo (not worktree) after the PR is merged.
    Updates are made directly on the main branch to prevent cross-contamination
    ...
    Raises:
        Exception: If KPI tracking or commit/push fails
    """
    from asw.modules.data_types import AgentTemplateRequest
    from asw.modules.agent import execute_template

    repo_root = get_main_repo_root()
    logger.info(f"Tracking KPIs on main branch in: {repo_root}")

    # Ensure we're on main and up to date
    logger.info("Ensuring main branch is up to date...")
    subprocess.run(["git", "checkout", "main"], cwd=repo_root, check=True)
    subprocess.run(["git", "pull", "origin", "main"], cwd=repo_root, check=True)

    # Run KPI tracking agent
    logger.info("Executing KPI tracker agent...")
    ...
    if result.stdout.strip():
        # Commit and push KPI updates
        logger.info("Committing and pushing KPI updates...")
        subprocess.run(
            ["git", "add", "app_docs/agentic_kpis.md"],
            cwd=repo_root,
            check=True
        )
        subprocess.run(
            ["git", "commit", "-m", f"kpi_tracker: update KPIs for {adw_id}"],
            cwd=repo_root,
            check=True
        )
        subprocess.run(
        ...
        logger.info("KPI updates committed and pushed to main")
    else:
        logger.info("No KPI changes to commit")


def generate_ipe_id_from_adw(adw_id: str) -> str:
    """Generate IPE ID from ADW ID for AMI builds.

    Uses 'ami-' prefix + first 5 chars of adw_id.
    Example: abc123de -> ami-abc12

    Args:
        adw_id: The ADW ID (e.g., "ADW-abc123de" or "abc123de")

    Returns:
        IPE ID for AMI build (e.g., "ami-abc12")
    """
    base_id = adw_id.replace("ADW-", "").replace("adw-", "")
...
def create_ipe_state_for_ami_build(
    ...
    """Create IPE state from ADW state for AMI building.

    Args:
        adw_state: ADW state object
    ...
    try:
        ...
        ipe_state = IPEState(ipe_id)
        ipe_state.update(
            issue_number=adw_state.get("issue_number"),
            branch_name=adw_state.get("branch_name"),
            worktree_path=adw_state.get("worktree_path"),
            environment=environment,
            issue_class="/ipe_feature",
            spec_file=adw_state.get("plan_file"),
            model_set=adw_state.get("model_set", "base"),
        )

        ipe_state.save("adw_ship_iso_ami_integration")
    ...
    try:
        ...
        if not is_clean:
            ...
            success, error, stash_id = stash_changes(
                message=f"ASW App Ship: Auto-stash before merging {branch_name}",
                cwd=repo_root,
                logger=logger
            )
        ...
        result = subprocess.run(
            ["git", "merge", branch_name, "--no-ff", "-m", f"Merge branch '{branch_name}' via ASW App Ship workflow"],
            capture_output=True, text=True, cwd=repo_root
        )
        if result.returncode != 0:
...
def main():
    ...
    if len(sys.argv) < 3:
        print("Usage: uv run asw_app_ship_iso.py <issue-number> <asw-id> [--skip-ci-check] [--create-ami]")
    ...
    state = ASWAppState.load(adw_id, temp_logger)
    if not state:
    ...
    logger = setup_logger(adw_id, "adw_ship_iso")
    logger.info(f"ASW App Ship Iso starting - ID: {adw_id}, Issue: {issue_number}")
    if skip_ci_check:
    ...
    if not is_valid:
        ...
        make_issue_comment(
            ...
            format_issue_message(adw_id, AGENT_SHIPPER, f"❌ {error_msg}\n\n"
                               ...
                               "- asw_app_plan_iso.py (creates plan_file, branch_name, issue_class)\n"
                               "- asw_app_build_iso.py (implements the plan)\n" 
                               "- asw_app_test_iso.py (runs tests)\n"
                               "- asw_app_review_iso.py (reviews implementation)\n"
                               "- asw_app_document_iso.py (generates docs)")
        )
        sys.exit(1)
    ...
    # Step 4.5: Track KPIs on main branch after successful merge
    logger.info("Tracking agentic KPIs on main branch...")
    make_issue_comment(
        issue_number,
        format_issue_message(adw_id, AGENT_SHIPPER, "📊 Updating agentic KPIs on main branch")
    )

    try:
        track_kpis_on_main(issue_number, adw_id, state, logger)
        logger.info("Successfully updated agentic KPIs")
    except Exception as e:
        # Don't fail the ship workflow if KPI tracking fails
        logger.warning(f"Failed to track KPIs (non-fatal): {e}")
        make_issue_comment(
            issue_number,
            format_issue_message(adw_id, AGENT_SHIPPER, f"⚠️ KPI tracking failed (non-fatal): {e}")
        )
    ...
    state.append_asw_id("adw_ship_iso")
    ...
    if create_ami:
        ...
        ipe_id = generate_ipe_id_from_adw(adw_id)
    ...
    make_issue_comment(
        issue_number,
        format_issue_message(adw_id, AGENT_SHIPPER, success_message)
    )
