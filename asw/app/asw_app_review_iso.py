...
"""
ASW App Review Iso - AI Developer Workflow for agentic review in isolated worktrees

Usage:
  uv run asw_app_review_iso.py <issue-number> <asw-id> [--skip-resolution]
...
This workflow REQUIRES that asw_app_plan_iso.py or asw_app_patch_iso.py has been run first
...
from asw.modules.utils import setup_logger, parse_json, check_env_vars
...
def upload_review_screenshots(
    ...
    for i, local_path in enumerate(review_result.screenshots):
        ...
        if is_base64:
            ...
            remote_path = f"adw/{adw_id}/review/{filename}"
...
def main():
    ...
    if len(sys.argv) < 3:
        print("Usage: uv run asw_app_review_iso.py <issue-number> <asw-id> [--skip-resolution]")
        print("\nError: adw-id is required to locate the worktree")
        print("Run asw_app_plan_iso.py or asw_app_patch_iso.py first to create the worktree")
        sys.exit(1)
    ...
    if state:
        ...
        logger.error("Run asw_app_plan_iso.py or asw_app_patch_iso.py first to create the worktree and state")
        print(f"\nError: No state found for ADW ID: {adw_id}")
        print("Run asw_app_plan_iso.py or asw_app_patch_iso.py first to create the worktree and state")
        sys.exit(1)
    
    # Track that this ADW workflow has run
    state.append_asw_id("adw_review_iso")
    
    # Set up logger with ADW ID from command line
    logger = setup_logger(adw_id, "adw_review_iso")
    logger.info(f"ASW App Review Iso starting - ID: {adw_id}, Issue: {issue_number}, Skip Resolution: {skip_resolution}")
    ...
    if not valid:
        ...
        logger.error("Run asw_app_plan_iso.py or asw_app_patch_iso.py first")
        make_issue_comment(
            issue_number,
            format_issue_message(adw_id, "ops", f"❌ Worktree validation failed: {error}\n"
                               "Run asw_app_plan_iso.py or asw_app_patch_iso.py first")
        )
        sys.exit(1)
