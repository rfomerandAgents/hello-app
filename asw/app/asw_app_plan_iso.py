...
"""
ASW App Plan Iso - AI Developer Workflow for agentic planning in isolated worktrees

Usage:
  uv run asw_app_plan_iso.py <issue-number> [asw-id] [--cache|--no-cache]
...
from asw.modules.utils import setup_logger, check_env_vars, parse_cache_flag
from asw.modules.data_types import GitHubIssue, IssueClassSlashCommand, AgentTemplateRequest
...
def validate_plan_path(plan_path: str, worktree_path: str, logger: logging.Logger) -> str:
    """Ensure plan path is relative or points to worktree, not main repo.

    Args:
        plan_path: Path returned by planning agent (could be relative or absolute)
        worktree_path: Absolute path to the worktree
        logger: Logger instance for warnings

    Returns:
        Corrected relative path that points to worktree
    """
    # If already relative, it's correct
    if not os.path.isabs(plan_path):
        return plan_path

    # If absolute, check if it points to the main repo instead of worktree
    # Main repo path is the parent directory of 'trees/'
    if '/trees/' in worktree_path:
        main_repo = worktree_path.split('/trees/')[0]

        # If absolute path points to main repo (no /trees/ in it), convert to relative
        if plan_path.startswith(main_repo) and '/trees/' not in plan_path:
            relative = plan_path.replace(main_repo + '/', '')
            logger.warning(f"Plan path corrected from main repo to relative: {plan_path} -> {relative}")
            return relative

    # If absolute path points to worktree, convert to relative
    if plan_path.startswith(worktree_path + '/'):
        relative = plan_path.replace(worktree_path + '/', '')
        logger.warning(f"Plan path converted from absolute to relative: {plan_path} -> {relative}")
        return relative
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: uv run asw_app_plan_iso.py <issue-number> [asw-id] [--cache|--no-cache]")
        sys.exit(1)
    ...
    adw_id = ensure_asw_app_id(issue_number, adw_id, temp_logger)
    ...
    state.append_asw_id("adw_plan_iso")

    # Set up logger with ADW ID
    logger = setup_logger(adw_id, "adw_plan_iso")
    logger.info(f"ASW App Plan Iso starting - ID: {adw_id}, Issue: {issue_number}")
    ...
    if valid:
        ...
        backend_port, frontend_port = get_ports_for_asw(adw_id)
    ...
    # Classify the issue
    issue_command, error = classify_issue(issue, adw_id, logger, cache_enabled)
    ...
    branch_name, error = generate_branch_name(issue, issue_command, adw_id, logger, cache_enabled)
    ...
    plan_response = build_plan(issue, issue_command, adw_id, logger, working_dir=worktree_path, cache_enabled=cache_enabled)
    ...
    # Validate and correct plan path if needed (defensive)
    plan_file_path = validate_plan_path(plan_file_path, worktree_path, logger)
    ...
    commit_msg, error = create_commit(
        AGENT_PLANNER, issue, issue_command, adw_id, logger, worktree_path, cache_enabled
    )
