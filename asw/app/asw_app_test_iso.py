...
"""
ASW App Test Iso - AI Developer Workflow for agentic testing in isolated worktrees

Usage:
  uv run asw_app_test_iso.py <issue-number> <asw-id> [--skip-e2e] [--cache|--no-cache]
...
from asw.modules.utils import make_adw_id, setup_logger, parse_json, check_env_vars, parse_cache_flag
...
def run_tests(adw_id: str, logger: logging.Logger, working_dir: Optional[str] = None, cache_enabled: bool = True) -> AgentPromptResponse:
...
def run_e2e_tests(adw_id: str, logger: logging.Logger, working_dir: Optional[str] = None, cache_enabled: bool = True) -> AgentPromptResponse:
...
def run_tests_with_resolution(
    ...
    while attempt < max_attempts:
        ...
        test_response = run_tests(adw_id, logger, worktree_path, cache_enabled)
        ...
        resolved, unresolved = resolve_failed_tests(
            failed_tests, adw_id, issue_number, logger, worktree_path, iteration=attempt, cache_enabled=cache_enabled
        )
...
def run_e2e_tests_with_resolution(
    ...
    while attempt < max_attempts:
        ...
        e2e_response = run_e2e_tests(adw_id, logger, worktree_path, cache_enabled)
...
def main():
    ...
    if len(sys.argv) < 3:
        print("Usage: uv run asw_app_test_iso.py <issue-number> <asw-id> [--skip-e2e] [--cache|--no-cache]")
    ...
    state.append_asw_id("adw_test_iso")
    
    # Set up logger with ADW ID from command line
    logger = setup_logger(adw_id, "adw_test_iso")
    logger.info(f"ASW App Test Iso starting - ID: {adw_id}, Issue: {issue_number}, Skip E2E: {skip_e2e}")
    ...
    results, passed_count, failed_count, test_response = run_tests_with_resolution(
        adw_id, issue_number, logger, worktree_path, cache_enabled=cache_enabled
    )
    ...
    commit_msg, error = create_commit(AGENT_TESTER, issue, issue_command, adw_id, logger, worktree_path, cache_enabled)
