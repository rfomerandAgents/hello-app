...
# /// script
# dependencies = ["python-dotenv", "pydantic", "boto3>=1.26.0", "claude-agent-sdk"]
# ///

"""
ASW IO Review Iso - Infrastructure Platform Engineer for agentic review in isolated worktrees

Usage:
  uv run asw_io_review_iso.py <issue-number> <adw-id> [--skip-resolution]
...
from ipe.ipe_modules.ipe_utils import setup_logger, parse_json, check_env_vars
...
# Import R2Uploader from unified ASW modules
from asw.modules import R2Uploader
...
def run_review(
    ...
    request = AgentTemplateRequest(
        ...
        slash_command="/ipe_review",
        args=[ipe_id, spec_file, AGENT_REVIEWER],
        ipe_id=ipe_id,
...
def upload_review_screenshots(
    ...
    Handles both file paths and base64 data URIs.
    ...
    for i, local_path in enumerate(review_result.screenshots):
        ...
        is_base64, mime_type, base64_data = uploader._parse_base64_data_uri(local_path)

        if is_base64:
            # Generate filename from index
            ext = mime_type.split("/")[-1] if mime_type else "png"
            filename = f"{i+1:02d}_screenshot.{ext}"
            remote_path = f"adw/{ipe_id}/review/{filename}"
            url = uploader.upload_base64(base64_data, remote_path, mime_type)
        else:
            # Handle as file path
            ...
            remote_path = f"adw/{ipe_id}/review/{os.path.basename(local_path)}"
        ...
        if url:
            ...
            logger.error(f"Failed to upload screenshot: {local_path[:50]}...")
    ...
    for issue in review_result.review_issues:
        if issue.screenshot_path:
            is_base64_issue, _, _ = uploader._parse_base64_data_uri(issue.screenshot_path)
            if is_base64_issue:
                # For base64, match by index in screenshots list
                for idx, local_path in enumerate(review_result.screenshots):
                    if local_path == issue.screenshot_path and idx < len(screenshot_urls):
                        issue.screenshot_url = screenshot_urls[idx]
                        break
            else:
                # For file paths, match by path
                for idx, local_path in enumerate(review_result.screenshots):
                    if local_path == issue.screenshot_path and idx < len(screenshot_urls):
                        issue.screenshot_url = screenshot_urls[idx]
                        break
...
def resolve_blocker_issues(
    ...
    for i, issue in enumerate(blocker_issues, 1):
        ...
        # Create patch plan
        plan_response = create_review_patch_plan(issue, i, ipe_id, logger, working_dir=worktree_path)
        ...
        spec_file = plan_response.output.strip()
        
        # Implement the patch
        logger.info(f"Implementing patch from plan: {spec_file}")
        impl_response = implement_plan(spec_file, ipe_id, logger, working_dir=worktree_path)
...
def main():
    ...
    if len(sys.argv) < 3:
        print("Usage: uv run asw_io_review_iso.py <issue-number> <adw-id> [--skip-resolution]")
    ...
    temp_logger = setup_logger(ipe_id, "ipe_review_iso")
    ...
    if state:
        ...
        make_issue_comment(
            issue_number,
            f"{ipe_id}_ops: 🔍 Found existing state - starting isolated review\n```json\n{json.dumps(state.data, indent=2)}\n```"
        )
    else:
        # No existing state found
        logger = setup_logger(ipe_id, "ipe_review_iso")
    ...
    state.append_asw_id("ipe_review_iso")
    
    # Set up logger with IPE ID from command line
    logger = setup_logger(ipe_id, "ipe_review_iso")
    logger.info(f"ASW IO Review Iso starting - ID: {ipe_id}, Issue: {issue_number}, Skip Resolution: {skip_resolution}")
    ...
    make_issue_comment(
        issue_number, 
        format_issue_message(ipe_id, "ops", f"✅ Starting isolated review phase\n"
    ...
    make_issue_comment(
        issue_number,
        format_issue_message(ipe_id, "ops", f"📋 Found spec file: {spec_file}")
    )
    ...
    while review_attempt < MAX_REVIEW_RETRY_ATTEMPTS:
        ...
        review_result = run_review(spec_file, ipe_id, logger, working_dir=worktree_path)
        ...
        resolve_blocker_issues(blocker_issues, issue_number, ipe_id, worktree_path, logger)
    ...
    if review_result:
        ...
        upload_review_screenshots(review_result, ipe_id, worktree_path, logger)
        logger.info(f"Uploaded {len(review_result.screenshot_urls)} screenshots to R2")

        # Build and post the summary comment
        summary = build_review_summary(review_result)
        make_issue_comment(
            issue_number,
            format_issue_message(ipe_id, AGENT_REVIEWER, summary)
        )
    ...
    commit_msg, error = create_commit(AGENT_REVIEWER, issue, issue_command, ipe_id, logger, worktree_path)
    
    if error:
        logger.error(f"Error creating commit message: {error}")
        make_issue_comment(
            issue_number,
            format_issue_message(ipe_id, AGENT_REVIEWER, f"❌ Error creating commit message: {error}")
        )
        sys.exit(1)
    ...
    if not success:
        ...
        make_issue_comment(
            issue_number,
            format_issue_message(ipe_id, AGENT_REVIEWER, f"❌ Error committing review: {error}")
        )
        sys.exit(1)
    
    logger.info(f"Committed review: {commit_msg}")
    make_issue_comment(
        issue_number, format_issue_message(ipe_id, AGENT_REVIEWER, "✅ Review committed")
    )
    ...
    make_issue_comment(
        issue_number, format_issue_message(ipe_id, "ops", "✅ Isolated review phase completed")
    )
    
    # Save final state
    state.save("ipe_review_iso")
    
    # Post final state summary to issue
    make_issue_comment(
        issue_number,
        f"{ipe_id}_ops: 📋 Final review state:\n```json\n{json.dumps(state.data, indent=2)}\n```"
    )
