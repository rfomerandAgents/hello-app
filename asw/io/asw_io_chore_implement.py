...
2. /ipe_build - Implements the plan created by /chore

Usage:
    # Method 1: Direct execution (requires uv)
    ./ipe/ipe_chore_implement.py "Add error handling to all API endpoints"

    # Method 2: Using uv run
    uv run ipe/ipe_chore_implement.py "Refactor database connection logic"

Examples:
    # Run with specific model
    ./ipe/ipe_chore_implement.py "Add logging to agent.py" --model opus

    # Run from a different working directory
    ./ipe/ipe_chore_implement.py "Update documentation" --working-dir /path/to/project

    # Run with verbose output
    ./ipe/ipe_chore_implement.py "Add tests" --verbose
"""
...
# Add the ipe_modules directory to the path so we can import agent
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "ipe_modules"))
...
def main(
    ...
    console.print(
        Panel(
            f"[bold blue]ASW IO Chore & Implement Workflow[/bold blue]\n\n"
            f"[cyan]IPE ID:[/cyan] {ipe_id}\n"
    ...
    chore_info_table.add_row("IPE ID", ipe_id)
    chore_info_table.add_row("IPE Name", "ipe_chore_implement (planning)")
    chore_info_table.add_row("Command", "/chore")
    chore_info_table.add_row("Args", f'{ipe_id} "{prompt}"')
    ...
    try:
        ...
        chore_output_dir = f"./agents/{ipe_id}/{planner_name}"
        ...
        # Phase 2: Run /ipe_build command
        console.print(
            Rule("[bold yellow]Phase 2: Implementation (/ipe_build)[/bold yellow]")
        )
        console.print()
        ...
        implement_info_table.add_row("IPE ID", ipe_id)
        implement_info_table.add_row("IPE Name", "ipe_chore_implement (building)")
        implement_info_table.add_row("Command", "/ipe_build")
        ...
        implement_output_dir = f"./agents/{ipe_id}/{builder_name}"
            ...
            json.dump(
                {
                    ...
                    "slash_command": "/ipe_build",
                    "args": [plan_path],
                    "path_to_slash_command_prompt": ".claude/commands/ipe_build.md",
                    "model": model,
                    "working_dir": working_dir,
        ...
        summary_table.add_row(
            "Planning (/chore)",
            planning_status,
            f"./agents/{ipe_id}/{planner_name}/",
        )

        # Implementation phase row
        implement_status = "✅ Success" if implement_response.success else "❌ Failed"
        summary_table.add_row(
            "Implementation (/ipe_build)",
            implement_status,
            f"./agents/{ipe_id}/{builder_name}/",
        )

        console.print(summary_table)

        # Create overall workflow summary
        workflow_summary_path = f"./agents/{ipe_id}/workflow_summary.json"
        os.makedirs(f"./agents/{ipe_id}", exist_ok=True)
            ...
            json.dump(
                {
                    ...
                    "phases": {
                        "planning": {
                            ...
                            "session_id": chore_response.session_id,
                            "agent": planner_name,
                            "output_dir": f"./agents/{ipe_id}/{planner_name}/",
                        },
                        "implementation": {
                            "success": implement_response.success,
                            "session_id": implement_response.session_id,
                            "agent": builder_name,
                            "output_dir": f"./agents/{ipe_id}/{builder_name}/",
                        },
                    },
