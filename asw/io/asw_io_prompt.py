...
Usage:
    ...
    ./ipe_prompt.py "Write a hello world Python script"

    # Method 2: Using uv run
    uv run asw_io_prompt.py "Write a hello world Python script"

    # Method 3: Using Python directly (requires dependencies installed)
    python ipe_prompt.py "Write a hello world Python script"

Examples:
    # Run with specific model
    ./ipe_prompt.py "Explain this code" --model opus

    # Run with custom output file
    ./ipe_prompt.py "Create a FastAPI app" --output my_result.jsonl

    # Run from a different working directory
    ./ipe_prompt.py "List files here" --working-dir /path/to/project

    # Disable retry on failure
    ./ipe_prompt.py "Quick test" --no-retry

    # Use custom agent name
    ./ipe_prompt.py "Debug this" --agent-name debugger
"""
...
def main(
    ...
    if not output:
        # Default: write to agents/<ipe_id>/<agent_name>/
        output_dir = Path(f"./agents/{ipe_id}/{agent_name}")
    ...
    info_table.add_row("IPE ID", ipe_id)
    info_table.add_row("IPE Name", "ipe_prompt")
