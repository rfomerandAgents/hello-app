...
"""
ASW IO Patch Isolated - Infrastructure Platform Engineer for single-issue patches with worktree isolation

Usage:
  uv run asw_io_patch_iso.py <issue-number> [adw-id]
...
4. Check for 'ipe_patch' keyword in comments or issue body
5. Create patch plan based on content containing 'ipe_patch'
6. Implement the patch plan
7. Commit changes
8. Push and create/update PR

This workflow requires 'ipe_patch' keyword to be present either in:
- A comment on the issue (uses latest comment containing keyword)
- The issue body itself (uses issue title + body)

Key features:
- Runs in isolated git worktree under trees/<asw_id>/
...
from asw.modules.git_ops import commit_changes, finalize_git_operations
...
from asw.modules.workflow_ops import (
    create_commit,
...
from asw.modules.worktree_ops import (
    create_worktree,
    validate_worktree,
    get_ports_for_adw,
    is_port_available,
...
from asw.modules.agent import execute_template
...
def get_patch_content(
    issue: GitHubIssue, issue_number: str, ipe_id: str, logger: logging.Logger
) -> str:
    """Get patch content from either issue comments or body containing 'ipe_patch'.

    Args:
        issue: The GitHub issue
        issue_number: Issue number for comments
        ipe_id: IPE ID for formatting messages
    ...
    Raises:
        SystemExit: If 'ipe_patch' keyword is not found
    """
    # First, check for the latest comment containing 'ipe_patch'
    keyword_comment = find_keyword_from_comment("ipe_patch", issue)

    if keyword_comment:
        # Use the comment body as the review change request
        logger.info(
            f"Found 'ipe_patch' in comment, using comment body: {keyword_comment.body}"
        )
        ...
        make_issue_comment(
            ...
            format_issue_message(
                ipe_id,
                AGENT_PATCH_PLANNER,
                f"✅ Creating patch plan from comment containing 'ipe_patch':\n\n```\n{keyword_comment.body}\n```",
            ),
        )
        return review_change_request
    elif "ipe_patch" in issue.body:
        # Use issue title and body as the review change request
        logger.info("Found 'ipe_patch' in issue body, using issue title and body")
        ...
        make_issue_comment(
            ...
            format_issue_message(
                ipe_id,
                AGENT_PATCH_PLANNER,
                "✅ Creating patch plan from issue containing 'ipe_patch'",
            ),
        )
        return review_change_request
    else:
        # No 'ipe_patch' keyword found, exit
        logger.error("No 'ipe_patch' keyword found in issue body or comments")
        make_issue_comment(
            issue_number,
            format_issue_message(
                ipe_id,
                "ops",
                "❌ No 'ipe_patch' keyword found in issue body or comments. Add 'ipe_patch' to trigger patch workflow.",
            ),
        )
        sys.exit(1)
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: uv run asw_io_patch_iso.py <issue-number> [adw-id]")
        sys.exit(1)
    ...
    temp_logger = setup_logger(ipe_id, "ipe_patch_iso") if ipe_id else None
    ...
    state.append_asw_id("ipe_patch_iso")

    # Set up logger with IPE ID
    logger = setup_logger(ipe_id, "ipe_patch_iso")
    logger.info(f"ASW IO Patch Isolated starting - ID: {ipe_id}, Issue: {issue_number}")
    ...
    make_issue_comment(
        issue_number,
        format_issue_message(ipe_id, "ops", "✅ Starting isolated patch workflow"),
    )
    ...
    if not branch_name:
        ...
        from asw.modules.workflow_ops import find_existing_branch_for_issue

        existing_branch = find_existing_branch_for_issue(issue_number, ipe_id)
        ...
        if existing_branch:
            ...
            # Classify the issue
            from asw.modules.workflow_ops import classify_issue
            ...
            if error:
                ...
                make_issue_comment(
                    issue_number,
                    format_issue_message(
                        ipe_id, "ops", f"❌ Failed to classify issue: {error}"
                    ),
                )
                sys.exit(1)
            ...
            from asw.modules.workflow_ops import generate_branch_name

            branch_name, error = generate_branch_name(
                issue, issue_command, ipe_id, logger
            )
            if error:
    ...
    state.save("ipe_patch_iso")
    logger.info(f"Working on branch: {branch_name}")
    make_issue_comment(
        issue_number,
        format_issue_message(ipe_id, "ops", f"✅ Working on branch: {branch_name}"),
    )
    ...
    if worktree_path and os.path.exists(worktree_path):
        ...
        if error:
            ...
            make_issue_comment(
                issue_number,
                format_issue_message(
                    ipe_id, "ops", f"❌ Error creating worktree: {error}"
                ),
            )
            sys.exit(1)

        # Get deterministic ports for this IPE ID
    ...
    # Get patch content from issue or comments containing 'ipe_patch'
    logger.info("Checking for 'ipe_patch' keyword")
    review_change_request = get_patch_content(issue, issue_number, ipe_id, logger)
    ...
    if not patch_file:
        ...
        make_issue_comment(
            issue_number,
            format_issue_message(
                ipe_id, AGENT_PATCH_PLANNER, "❌ Failed to create patch plan"
            ),
        )
        sys.exit(1)
    ...
    make_issue_comment(
        issue_number,
        format_issue_message(
            ipe_id, AGENT_PATCH_PLANNER, f"✅ Patch plan created: {patch_file}"
        ),
    )
    ...
    make_issue_comment(
        issue_number,
        format_issue_message(ipe_id, AGENT_PATCH_IMPLEMENTOR, "✅ Patch implemented"),
    )
    ...
    commit_msg, error = create_commit(
        AGENT_PATCH_IMPLEMENTOR, issue, issue_command, ipe_id, logger, worktree_path
    )
    ...
    if not success:
        ...
        make_issue_comment(
            issue_number,
            format_issue_message(
                ipe_id, AGENT_PATCH_IMPLEMENTOR, f"❌ Error committing patch: {error}"
            ),
        )
        sys.exit(1)
    ...
    make_issue_comment(
        issue_number,
        format_issue_message(ipe_id, AGENT_PATCH_IMPLEMENTOR, "✅ Patch committed"),
    )

    logger.info("Finalizing git operations")
    make_issue_comment(
        issue_number,
        format_issue_message(ipe_id, "ops", "🔧 Finalizing git operations"),
    )
    ...
    make_issue_comment(
        issue_number,
        format_issue_message(ipe_id, "ops", "✅ Isolated patch workflow completed"),
    )
    ...
    make_issue_comment(
        issue_number,
        f"{ipe_id}_ops: 📋 Final isolated patch state:\n```json\n{json.dumps(state.data, indent=2)}\n```",
    )
