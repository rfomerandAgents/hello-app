...
"""
ASW IO SDLC ZTE Iso - Zero Touch Execution: Complete SDLC with automatic shipping

Usage: uv run asw_io_sdlc_zte_iso.py <issue-number> [adw-id] [--skip-e2e] [--skip-resolution]

This script runs the complete ASW IO SDLC pipeline with automatic shipping:
1. asw_io_plan_iso.py - Planning phase (isolated)
2. asw_io_build_iso.py - Implementation phase (isolated)
3. asw_io_test_iso.py - Testing phase (isolated)
4. asw_io_review_iso.py - Review phase (isolated)
5. asw_io_document_iso.py - Documentation phase (isolated)
6. asw_io_ship_iso.py - Ship phase (approve & merge PR)
...
from asw.modules.workflow_ops import ensure_asw_io_id, detect_full_deploy
...
def main():
    ...
    try:
        base_message = (
            f"{ipe_id}_ops: 🚀 **Starting Zero Touch Execution (ZTE)**\n\n"
    ...
    # Run isolated test with the IPE ID
    test_cmd = [
        "uv",
        "run",
    ...
    if test.returncode != 0:
        ...
        try:
            make_issue_comment(
                issue_number,
                f"{ipe_id}_ops: ❌ **ZTE Aborted** - Test phase failed\n\n"
    ...
    if review.returncode != 0:
        ...
        try:
            make_issue_comment(
                issue_number,
                f"{ipe_id}_ops: ❌ **ZTE Aborted** - Review phase failed\n\n"
    ...
    # Run isolated ship with the IPE ID
    ship_cmd = [
        "uv",
        "run",
        os.path.join(script_dir, "asw_io_ship_iso.py"),
        issue_number,
        ipe_id,
    ]
    ...
    if ship.returncode != 0:
        ...
        try:
            make_issue_comment(
                issue_number,
                f"{ipe_id}_ops: ❌ **ZTE Failed** - Ship phase failed\n\n"
    ...
    if enable_full_deploy:
        ...
        if validation_errors:
            ...
            try:
                make_issue_comment(
                    issue_number,
                    f"{ipe_id}_ops: ❌ **Full-Deploy Aborted**\n\n{error_msg}"
                )
            except:
                pass
            sys.exit(1)
        ...
        try:
            make_issue_comment(
                issue_number,
                f"{ipe_id}_ops: 🌟 **Starting Full-Deploy**\n\n"
        ...
        try:
            ...
            try:
                make_issue_comment(
                    issue_number,
                    f"{ipe_id}_ops: ⏱️ **Full-Deploy Timeout**\n\n{error_msg}\n"
        ...
        if build_result.returncode != 0:
            ...
            try:
                make_issue_comment(
                    issue_number,
                    f"{ipe_id}_ops: ❌ **Full-Deploy Failed**\n\n"
        ...
        try:
            make_issue_comment(
                issue_number,
                f"{ipe_id}_ops: ✅ **AMI Build Complete**\n\n"
        ...
        try:
            success_msg = (
                f"{ipe_id}_ops: 🎉 **Full-Deploy Completed Successfully!**\n\n"
            ...
            success_msg += (
                ...
                f"Worktree location: `trees/{ipe_id}/`\n"
                f"To clean up: `./scripts/purge_tree.sh {ipe_id}`"
            )
    ...
    if not enable_full_deploy:
        try:
            make_issue_comment(
                issue_number,
                f"{ipe_id}_ops: 🎉 **Zero Touch Execution Complete!**\n\n"
