...
"""
ASW IO Plan Build Test Iso - Compositional workflow for isolated planning, building, and validation

Usage: uv run asw_io_plan_build_test_iso.py <issue-number> [asw-id] [environment]

This script runs:
1. asw_io_plan_iso.py - Planning phase (isolated)
2. asw_io_build_iso.py - Implementation phase (isolated)
3. asw_io_test_iso.py - Validation phase (isolated)
...
def main():
    """Main entry point."""
    if len(sys.argv) < 2:
        print("Usage: uv run asw_io_plan_build_test_iso.py <issue-number> [asw-id] [environment]")
        print("\nThis runs the isolated plan, build, and validation workflow (defaults to sandbox):")
