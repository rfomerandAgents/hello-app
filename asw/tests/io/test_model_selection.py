#!/usr/bin/env python3
"""Test model selection functionality for IPE workflows."""
...
def test_model_mapping_lookups():
    ...
    test_cases = [
        ...
        ("/ipe_build", "base", "sonnet"),
        ("/ipe_build", "heavy", "opus"),
...
def test_get_model_for_slash_command():
    ...
    # Create a mock IPE state
    from asw.modules.state import ASWIOState
    
    # Test with base model set
    test_ipe_id = "test1234"
    state = ASWIOState(test_ipe_id)
    ...
    if model == expected_base:
        print(f"✅ With model_set='base': /ipe_build → {model}")
    else:
        print(f"❌ With model_set='base': /ipe_build → {model} (expected {expected_base})")
    ...
    if model == expected_heavy:
        print(f"✅ With model_set='heavy': /ipe_build → {model}")
    else:
        print(f"❌ With model_set='heavy': /ipe_build → {model} (expected {expected_heavy})")
    ...
    state_file = os.path.join(
        ...
        "agents", test_ipe_id, "ipe_state.json"
    )
...
def main():
    """Run all tests."""
    print("IPE Model Selection Tests")
    print("=" * 50)
