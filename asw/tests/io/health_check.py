...
"""
Health Check Script for IPE System

Usage:
uv run ipe/health_check.py <issue_number>
...
from ipe.ipe_modules.ipe_github import get_repo_url, extract_repo_path, make_issue_comment
from ipe.ipe_modules.ipe_utils import get_safe_subprocess_env
...
def check_env_vars() -> CheckResult:
    ...
    optional_vars = {
        "GITHUB_PAT": "(Optional) GitHub Personal Access Token - only needed if you want IPE to use a different GitHub account than 'gh auth login'",
...
def run_health_check() -> HealthCheckResult:
    ...
    if os.getenv("CLAUDE_CODE_OAUTH_TOKEN"):
        ...
        result.checks["claude_code"] = CheckResult(
            success=False,
            details={"skipped": True, "reason": "CLAUDE_CODE_OAUTH_TOKEN not set"},
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="IPE System Health Check")
    parser.add_argument(
        "issue_number",
        nargs="?",
    ...
    print("🏥 Running IPE System Health Check...\n")
    ...
    if not result.success:
        print("\n📝 Next Steps:")
        if any("CLAUDE_CODE_OAUTH_TOKEN" in e for e in result.errors):
            print("   1. Set CLAUDE_CODE_OAUTH_TOKEN in your .env file")
