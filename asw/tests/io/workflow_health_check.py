...
"""
Workflow Health Check - Monitor IPE workflow states and git status

Usage:
    uv run ipe/ipe_tests/workflow_health_check.py [--auto-fix]
...
def check_stuck_workflows() -> List[WorkflowHealthIssue]:
    ...
    for ipe_dir in agents_dir.iterdir():
        if not ipe_dir.is_dir():
            continue

        state_file = ipe_dir / "ipe_state.json"
        ...
        try:
            ...
            ipe_id = state_data.get("ipe_id")
            all_ipes = state_data.get("all_ipes", [])

            # Check if ship workflow completed
            if "ipe_ship_iso" not in all_ipes:
                # Determine age of workflow
                created_time = ipe_dir.stat().st_ctime
                ...
                if age_hours > 24:  # Stuck for more than 24 hours
                    issues.append(WorkflowHealthIssue(
                        ...
                        description=f"Workflow {ipe_id} has been incomplete for {age_hours:.1f} hours",
                        ipe_id=ipe_id,
                        auto_fixable=False
                    ))
            ...
            issues.append(WorkflowHealthIssue(
                ...
                description=f"Failed to read state for {ipe_dir.name}: {e}",
                ipe_id=ipe_dir.name,
                auto_fixable=False
            ))
...
def check_orphaned_worktrees() -> List[WorkflowHealthIssue]:
    ...
    for worktree_dir in trees_dir.iterdir():
        ...
        ipe_id = worktree_dir.name
        state_file = agents_dir / ipe_id / "ipe_state.json"

        if not state_file.exists():
            issues.append(WorkflowHealthIssue(
                severity="warning",
                category="orphaned_worktree",
                description=f"Worktree exists without state file: {ipe_id}",
                ipe_id=ipe_id,
                auto_fixable=True,
                fix_command=f"./scripts/purge_tree.sh {ipe_id}"
            ))
