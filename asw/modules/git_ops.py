"""Git operations for ASW composable architecture.
...
# Note: specs/issue-*-asw-* uses glob pattern for ASW-generated spec files
WORKTREE_EXCLUDED_FILES = [
    ".mcp.json",
    ".ports.env",
    "playwright-mcp-config.json",
    ".playwright-mcp/",  # Playwright MCP screenshots - worktree-specific test artifacts
    "specs/issue-*-asw-*",  # ASW spec files - ephemeral contracts, not permanent artifacts
    "specs/issue-*-adw-*",  # Legacy ADW spec files
...
def approve_pr(pr_number: str, logger: logging.Logger, workflow_type: str = "app") -> Tuple[bool, Optional[str]]:
    """Approve a PR. Returns (success, error_message).

    Args:
        pr_number: The PR number to approve
        logger: Logger instance
        workflow_type: Either "app" or "io" for the approval message
    """
    try:
        repo_url = get_repo_url()
        repo_path = extract_repo_path(repo_url)
    ...
    workflow_name = "ASW App" if workflow_type == "app" else "ASW IO"
    ...
    result = subprocess.run(
        [
            ...
            "--approve",
            "--body",
            f"{workflow_name} Ship workflow approved this PR after validating all state fields.",
        ],
        capture_output=True,
        text=True,
    )
...
def merge_pr(
    pr_number: str, logger: logging.Logger, merge_method: str = "squash", workflow_type: str = "app"
) -> Tuple[bool, Optional[str]]:
    ...
    Args:
        ...
        workflow_type: Either "app" or "io" for the merge message
    """
    try:
        repo_url = get_repo_url()
        repo_path = extract_repo_path(repo_url)
    ...
    # Add auto-merge body
    merge_cmd.extend(
        ["--body", f"Merged by {workflow_name} Ship workflow after successful validation."]
    )
...
def finalize_git_operations(
    state, logger: logging.Logger, cwd: Optional[str] = None, workflow_type: str = "app"
) -> None:
    """Standard git finalization: push branch and create/update PR.

    Args:
        state: ASWAppState or ASWIOState instance
        logger: Logger instance
        cwd: Optional working directory
        workflow_type: Either "app" or "io" for bot identifier
    """
    from .github import ASW_APP_BOT_IDENTIFIER, ASW_IO_BOT_IDENTIFIER
    ...
    if pr_url:
        ...
        if issue_number and asw_id:
            make_issue_comment(
                issue_number,
                f"{asw_id}_ops: Pull request: {pr_url}",
        ...
        if issue_number:
            try:
                ...
                pr_url, error = create_pull_request(branch_name, issue, state, logger, cwd, workflow_type)
            except Exception as e:
        ...
        if pr_url:
            ...
            if issue_number and asw_id:
                make_issue_comment(
                    issue_number,
                    f"{asw_id}_ops: Pull request created: {pr_url}",
                    bot_identifier=bot_identifier
                )
        else:
