"""Utility functions for ASW (Agentic Software Workflow) system.

Provides utilities for both application (app) and infrastructure (io) workflows.
"""
...
def make_asw_app_id() -> str:
    """Generate a short 8-character UUID for app workflow tracking.

    Replaces the former make_adw_id() function.
    """
    return str(uuid.uuid4())[:8]


def make_asw_io_id() -> str:
    """Generate a short 8-character UUID for IO workflow tracking.

    Replaces the former make_ipe_id() function.
    """
    return str(uuid.uuid4())[:8]
...
def setup_logger(asw_id: str, trigger_type: str = "asw_app_sdlc") -> logging.Logger:
    """Set up logger that writes to both console and file using asw_id.

    Args:
        asw_id: The ASW workflow ID
        trigger_type: Type of trigger (asw_app_sdlc, asw_io_sdlc, trigger_webhook, etc.)

    Returns:
        Configured logger instance
    """
    # Create log directory: agents/{asw_id}/{trigger_type}/
    # __file__ is in asw/modules/, so we need to go up 3 levels to get to project root
    project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    log_dir = os.path.join(project_root, "agents", asw_id, trigger_type)
    os.makedirs(log_dir, exist_ok=True)

    # Log file path: agents/{asw_id}/{trigger_type}/execution.log
    log_file = os.path.join(log_dir, "execution.log")

    # Create logger with unique name using asw_id
    ...
    logger.info(f"ASW Logger initialized - ID: {asw_id}")
    logger.debug(f"Log file: {log_file}")

    return logger


def get_logger(asw_id: str) -> logging.Logger:
    """Get existing logger by ASW ID.
    ...
    Returns:
        Logger instance
    """
    return logging.getLogger(f"asw_{asw_id}")
...
def get_safe_subprocess_env() -> Dict[str, str]:
    ...
    Returns only the environment variables needed for ASW workflows based on
...
def parse_cache_flag(argv: list) -> tuple[bool, list]:
    """Parse --cache/--no-cache flags from command line args.

    Args:
        argv: sys.argv list

    Returns:
        Tuple of (cache_enabled, cleaned_argv)
    """
    cache_enabled = False  # Default: caching off
    cleaned = argv.copy()

    if "--no-cache" in cleaned:
        cache_enabled = False
        cleaned.remove("--no-cache")
    elif "--cache" in cleaned:
        cache_enabled = True
        cleaned.remove("--cache")

    return cache_enabled, cleaned
