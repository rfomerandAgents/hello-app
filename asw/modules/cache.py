"""Claude response caching module for ASW workflows.
...
def create_fingerprint(
    ...
    Does NOT include:
    - asw_id (same prompt should cache across ASW instances)
...
def get_cache_dir(asw_id: str) -> Path:
    """Get cache directory path for an ASW instance."""
    project_root = Path(__file__).parent.parent.parent
    return project_root / "agents" / asw_id / "cache"


def get_cache_path(asw_id: str, fingerprint: str) -> Path:
    """Get full path for a cache file."""
    return get_cache_dir(asw_id) / f"{fingerprint}.json"


def save_cache_entry(asw_id: str, entry: CacheEntry) -> None:
    """Save a cache entry to disk."""
    cache_path = get_cache_path(asw_id, entry.fingerprint)
...
def load_cache_entry(asw_id: str, fingerprint: str) -> Optional[CacheEntry]:
    """Load a cache entry if it exists and is not expired."""
    cache_path = get_cache_path(asw_id, fingerprint)
...
def clear_cache(asw_id: str, max_age_seconds: Optional[int] = None) -> int:
    """Clear cache entries for an ASW instance.

    Args:
        asw_id: The ASW instance ID
        max_age_seconds: If provided, only clear entries older than this.

    Returns:
        Number of entries cleared
    """
    cache_dir = get_cache_dir(asw_id)
...
def get_cache_stats(asw_id: str) -> Dict[str, Any]:
    """Get statistics about cache for an ASW instance."""
    cache_dir = get_cache_dir(asw_id)
