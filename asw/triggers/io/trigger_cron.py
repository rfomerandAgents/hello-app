...
"""
Cron-based IPE trigger system that monitors GitHub issues and automatically processes them.
...
from asw.modules.utils import get_safe_subprocess_env

from asw.modules.github import fetch_open_issues, fetch_issue_comments, get_repo_url, extract_repo_path
...
def trigger_ipe_workflow(issue_number: int) -> bool:
    """Trigger the IPE plan and build workflow for a specific issue."""
    try:
        script_path = Path(__file__).parent.parent / "ipe_plan_build_iso.py"
        
        print(f"INFO: Triggering IPE workflow for issue #{issue_number}")
    ...
    try:
        ...
        if new_qualifying_issues:
            ...
            for issue_number in new_qualifying_issues:
                ...
                if trigger_ipe_workflow(issue_number):
...
def main():
    ...
    print(f"INFO: Starting IPE cron trigger")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] in ["--help", "-h"]:
        ...
        print("the IPE workflow for qualifying issues.")
