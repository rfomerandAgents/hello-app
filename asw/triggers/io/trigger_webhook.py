...
"""
GitHub Webhook Trigger - Infrastructure Platform Engineer (IPE)

FastAPI webhook endpoint that receives GitHub issue events and triggers IPE workflows.
...
- All workflow requirements (GITHUB_PAT, CLAUDE_CODE_OAUTH_TOKEN, etc.)
"""
...
from asw.modules.utils import make_ipe_id, setup_logger, get_safe_subprocess_env
from asw.modules.github import make_issue_comment, IPE_BOT_IDENTIFIER
from asw.modules.workflow_ops import extract_ipe_info, AVAILABLE_IPE_WORKFLOWS
...
# Create FastAPI app
app = FastAPI(
    title="IPE Webhook Trigger", description="GitHub webhook endpoint for IPE"
)

print(f"Starting IPE Webhook Trigger on port {PORT}")
...
async def github_webhook(request: Request):
    ...
    try:
        ...
        if event_type == "issues" and action == "opened" and issue_number:
            ...
            # Ignore issues from IPE bot to prevent loops
            if IPE_BOT_IDENTIFIER in issue_body:
                print(f"Ignoring IPE bot issue to prevent loop")
                workflow = None
            # Check if body contains "ipe_"
            elif "ipe_" in issue_body.lower():
                # Use temporary ID for classification
                temp_id = make_ipe_id()
                extraction_result = extract_ipe_info(issue_body, temp_id)
                if extraction_result.has_workflow:
                    workflow = extraction_result.workflow_command
                    provided_ipe_id = extraction_result.ipe_id
        ...
        # Check if this is an issue comment
            ...
            # Ignore comments from IPE bot to prevent loops
            if IPE_BOT_IDENTIFIER in comment_body:
                print(f"Ignoring IPE bot comment to prevent loop")
                workflow = None
            # Check if comment contains "ipe_"
            elif "ipe_" in comment_body.lower():
                # Use temporary ID for classification
                temp_id = make_ipe_id()
                extraction_result = extract_ipe_info(comment_body, temp_id)
                if extraction_result.has_workflow:
                    workflow = extraction_result.workflow_command
                    provided_ipe_id = extraction_result.ipe_id
                    model_set = extraction_result.model_set
        ...
        if workflow in DEPENDENT_WORKFLOWS:
            if not provided_ipe_id:
                print(
                    f"{workflow} is a dependent workflow that requires an existing IPE ID"
                )
                print(f"Cannot trigger {workflow} directly via webhook without IPE ID")
                workflow = None
                # Post error comment to issue
                try:
                    make_issue_comment(
                        str(issue_number),
                        f"❌ Error: `{workflow}` is a dependent workflow that requires an existing IPE ID.\n\n"
                        f"To run this workflow, you must provide the IPE ID in your comment, for example:\n"
                        f"`{workflow} adw-12345678`\n\n"
                        f"The IPE ID should come from a previous workflow run (like `ipe_plan_iso` or `ipe_patch_iso`).",
                    )
                except Exception as e:
                    print(f"Failed to post error comment: {e}")

        if workflow:
            # Use provided IPE ID or generate a new one
            ipe_id = provided_ipe_id or make_ipe_id()

            # If IPE ID was provided, update/create state file
            if provided_ipe_id:
                # Try to load existing state first
                state = ASWIOState.load(provided_ipe_id)
                if state:
                    ...
                    state = ASWIOState(provided_ipe_id)
                    state.update(
                        ipe_id=provided_ipe_id,
                        issue_number=str(issue_number),
                        model_set=model_set,
                    )
                state.save("webhook_trigger")
            else:
                # Create new state for newly generated IPE ID
                state = ASWIOState(ipe_id)
                state.update(
                    ipe_id=ipe_id, issue_number=str(issue_number), model_set=model_set
                )
                state.save("webhook_trigger")

            # Set up logger
            logger = setup_logger(ipe_id, "webhook_trigger")
            logger.info(
                f"Detected workflow: {workflow} from content: {content_to_check[:100]}..."
            )
            if provided_ipe_id:
                logger.info(f"Using provided IPE ID: {provided_ipe_id}")

            # Post comment to issue about detected workflow
            try:
                make_issue_comment(
                    str(issue_number),
                    f"🤖 IPE Webhook: Detected `{workflow}` workflow request\n\n"
                    f"Starting workflow with ID: `{ipe_id}`\n"
                    f"Workflow: `{workflow}` 🏗️\n"
                    f"Model Set: `{model_set}` ⚙️\n"
                    f"Reason: {trigger_reason}\n\n"
                    f"Logs will be available at: `agents/{ipe_id}/{workflow}/`",
                )
            except Exception as e:
            ...
            cmd = ["uv", "run", trigger_script, str(issue_number), ipe_id]
            ...
            print(
                f"Background process started for issue #{issue_number} with IPE ID: {ipe_id}"
            )
            print(f"Logs will be written to: agents/{ipe_id}/{workflow}/execution.log")
            ...
            return {
                ...
                "ipe_id": ipe_id,
                "workflow": workflow,
                "message": f"IPE {workflow} triggered for issue #{issue_number}",
                "reason": trigger_reason,
                "logs": f"agents/{ipe_id}/{workflow}/",
            }
        else:
            print(
...
async def health():
    ...
    try:
        ...
        script_dir = os.path.dirname(os.path.abspath(__file__))
        # Health check is in ipe_tests, not ipe_triggers
        health_check_script = os.path.join(
            os.path.dirname(script_dir), "ipe_tests", "health_check.py"
        )
        ...
        result = subprocess.run(
            ...
            text=True,
            timeout=30,
            cwd=os.path.dirname(script_dir),  # Run from project directory
        )
