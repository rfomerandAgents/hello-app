...
def classify_issue(
    issue: GitHubIssue, adw_id: str, logger: logging.Logger, cache_enabled: bool = True
    ...
    # Note: Match slash commands first, then standalone "0" with word boundaries
    # to avoid matching "0" within numbers like "800 × 600"
    classification_match = re.search(r"(/chore|/bug|/feature|/patch|/ipe_chore|/ipe_bug|/ipe_feature|/ipe_patch|\b0\b)", output)
    ...
    valid_commands = ["/chore", "/bug", "/feature", "/patch",
                      "/ipe_chore", "/ipe_bug", "/ipe_feature", "/ipe_patch"]
...
def generate_branch_name(
    ...
    # Extract branch name - handle cases where Claude adds explanation text
    # The branch name should be the last non-empty line matching the pattern
    output_lines = [line.strip() for line in response.output.strip().split('\n') if line.strip()]

    # Find the line that looks like a valid branch name (contains hyphens and looks git-friendly)
    branch_name = None
    for line in reversed(output_lines):
        # Skip markdown code fence markers
        if line in ['```', '```bash', '```text']:
            continue
        # Look for lines that match branch name pattern
        if '-issue-' in line and '-adw-' in line:
            branch_name = line
            break

    if not branch_name:
        # Fallback: use the last line if nothing matched
...
def detect_full_deploy(description: str) -> bool:
    ...
    Args:
        description: The text to search for the keyword

    Returns:
        True if 'full-deploy' or 'full deploy' is found (case-insensitive), False otherwise
    """
    if not description:
        return False
    # Case-insensitive pattern matching with word boundaries
    # Matches: "full-deploy", "full deploy", "FULL-DEPLOY", "Full Deploy"
