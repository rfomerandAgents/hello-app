...
SLASH_COMMAND_MODEL_MAP: Final[Dict[SlashCommand, Dict[ModelSet, str]]] = {
    ...
    "/classify_adw": {"base": "sonnet", "heavy": "opus"},
...
def prompt_claude_code(request: AgentPromptRequest) -> AgentPromptResponse:
    ...
    logger = logging.getLogger(f"adw_{request.adw_id}")
    if request.cache_enabled:
        # Create fingerprint for cache lookup
        fingerprint = create_fingerprint(
            prompt=request.prompt,
            model=request.model,
            working_dir=request.working_dir,
            slash_command=None,  # Not available at this level
        )

        # Try to load from cache
        cached = load_cache_entry(request.adw_id, fingerprint)
        ...
        if result.returncode == 0:
            ...
            if result_message:
                ...
                if request.cache_enabled and response.success:
                    ...
                    save_cache_entry(request.adw_id, cache_entry)
...
def execute_template(
    ...
    prompt_request = AgentPromptRequest(
        ...
        cache_enabled=request.cache_enabled,  # Pass through cache settings
